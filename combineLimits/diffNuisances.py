...
for i in range(fpf_s.getSize()):
	...
	# print mean_p,sigma_p,sigma_pu,sigma_pd
	mean_p =  sfv(mean_p)
	sigma_p = sfe(sigma_p)
	sigma_pu = sfe(sigma_pu)
	sigma_pd = -sfe(-sigma_pd)
	# print mean_p,sigma_p,sigma_pu,sigma_pd
    ...
    for fit_name, nuis_x in [('b', nuis_b), ('s',nuis_s)]:
	    ...
	    if fit_name=='b':
		    ...
		    if options.pullDef and nuis_p!=None:
		      ...
		      gr_fit_b.SetPoint(nuis_p_i-1,nuis_p_i-0.5+0.1,sfv(nx))
		      gr_fit_b.SetPointError(nuis_p_i-1,0,0,sfe(ned),sfe(neu))
		    else:
		      gr_fit_b.SetPoint(nuis_p_i-1,nuis_p_i-0.5+0.1,sfv(nuis_x.getVal()))
		      gr_fit_b.SetPointError(nuis_p_i-1,0,0,sfe(abs(nuis_x.getErrorLo())),sfe(nuis_x.getErrorHi()))
	      	    hist_fit_b.SetBinContent(nuis_p_i,sfv(nuis_x.getVal()))
	      	    hist_fit_b.SetBinError(nuis_p_i,sfe(nuis_x.getError()))
	      	    hist_fit_b.GetXaxis().SetBinLabel(nuis_p_i,name)
	      	    gr_fit_b.GetXaxis().SetBinLabel(nuis_p_i,name)
	          if fit_name=='s':
		    if options.pullDef and nuis_p!=None:
		      #nx,ned,neu = CP.returnPullAsym(options.pullDef,nuis_x.getVal(),mean_p,nuis_x.getErrorHi(),sigma_pu,abs(nuis_x.getErrorLo()),abs(sigma_pd))
		      gr_fit_s.SetPoint(nuis_p_i-1,nuis_p_i-0.5-0.1,sfv(nx))
		      gr_fit_s.SetPointError(nuis_p_i-1,0,0,sfe(ned),sfe(neu))
		    else:
		      gr_fit_s.SetPoint(nuis_p_i-1,nuis_p_i-0.5-0.1,sfv(nuis_x.getVal()))
		      gr_fit_s.SetPointError(nuis_p_i-1,0,0,sfe(abs(nuis_x.getErrorLo())),sfe(nuis_x.getErrorHi()))
	      	    hist_fit_s.SetBinContent(nuis_p_i,sfv(nuis_x.getVal()))
	      	    hist_fit_s.SetBinError(nuis_p_i,sfe(nuis_x.getError()))
...
if options.plotfile:
    ...
    hist_prefit.SetMaximum(2)
    hist_prefit.SetMinimum(0)
    hist_prefit.Draw("E2")
    hist_prefit.GetXaxis().SetLabelSize( hist_prefit.GetXaxis().GetLabelSize()*2.)
    hist_prefit.GetYaxis().SetLabelSize( hist_prefit.GetYaxis().GetLabelSize()*1.5)
    hist_prefit2 = hist_prefit.Clone()
    hist_prefit2.SetFillStyle(0)
    hist_prefit2.Draw("histsame")
