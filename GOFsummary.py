...
base+'templates_R17_GOF_deepjet_2DHTSF_v2'+tail,
base+'templates_R17_GOF_deepjet_no2DHTSF_v2'+tail,
...
variables=['thirdcsvb_bb','fourthcsvb_bb','NJetsCSV_MultiLepCalc','NJets_JetSubCalc','BDTtrijet2','AK4HTpMETpLepPt',
	...
	for r in rebins:
		for v in variables:
			...
			# entry['ndof']=cells[5]
				...
				entry['link'] = 'https://www.dropbox.com/home/fourtops/BDT/'+f.replace(base,'').replace(tail,'').replace('templates_','')+'?preview='+entry['variable']+'_'+lumi[entry['year']]+'_is'+entry['lepton']+'_nB'+entry['nB']+'_nJ'+entry['njets']+r+'_logy_totBand.png'
...
for year in ['R17']:
	...
	outfile = open("GOF_"+year+"_Mar2021.tsv", "w")
...
# outfile.write('Channel'+c+'prob_chi2'+c+'prob_chi2 (10pEvts)'+c+'prob_chi2 (no1stbin)'+c+'prob_chi2 (10pEvts+no1stbin)'+c+'ndof'+c+'ndof (10pEvts)'+c+'prob_chi2 (norebin)'+c+'prob_chi2 (norebin+no1stbin)'+c+'ndof (norebin)'+c+'link (norebin)'+c+'link (std rebin)'+c+'link (10pEvts rebin)'+r)
	outfile.write('Channel'+c+'prob_chi2 no 2DSF'+c+'prob_chi2 2DSF'+c+'ndof'+c+'link no 2DSF'+c+'link 2DSF'+r)
...
form(R_rebin_no2DHTSF['prob_chi2'])+c+#form(R_rebin_10pEvts['prob_chi2'])+c+
form(R_rebin_2DHTSF['prob_chi2'])+c+
...
str(R_rebin_no2DHTSF['ndof'])+c+#str(R_rebin_10pEvts['ndof'])+c+
